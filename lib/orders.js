...
orders.search = function* (table, orderBy, filter, startIndex, endIndex) {
  var conn, cursor, orders, ids = [], total;
  conn = yield connect();
  total = yield r.table(table).filter(filter).count().run(conn);
  cursor = yield r.table(table).orderBy(orderBy).filter(filter).slice(startIndex, endIndex).run(conn);
  ...
  cursor = yield r.table(table).orderBy(orderBy).limit(ids.length).filter(filter).changes().run(conn);
